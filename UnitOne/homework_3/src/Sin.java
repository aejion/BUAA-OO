...
public class Sin implements Factor {
    ...
    private String pp = "sin\\[(?<expression>.*?)\\]" + "(\\^" + allint + ")?";

    Sin(String str) { this.str = str; }

    @Override
    public int Iscorrect() {
        if (!str.substring(0,4).equals("sin(")) { return 0; }
        int len = str.length();
        int numofbr = 0;
    ...
    public String drivation() {
        if (str.equals("sin(x)")) { return "(cos(x))"; }
        int len = str.length();
        int numofbr = 0;
        StringBuilder str1 = new StringBuilder();
        int flag = 0;
        ...
        if (big == 0) { return "(0)"; }
        if (big == 1) { ans.append("cos(").append(str3).append(")"); }
        else if (big == 2) {
            ans.append("2*sin(").append(str3).append(")*cos(");
            ans.append(str3).append(")");
        }
        else {
            ans.append(big).append("*sin(").append(str3).append(")^").append(big - 1);
            ans.append("*cos(").append(str3).append(")");
        }
        Factor fac = ff.create(str3);
        if (!str3.equals("x")) { ans.append("*").append(fac.drivation()); }
        ans.append(")");
        return ans.toString();
    }
}
