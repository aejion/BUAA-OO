...
public class MyHeap<T extends Comparable<T>> {
    ...
    /*@ invariant (\forall int i; 0 <= i && 2 * i < elementData.length; elementData[i].compareTo(elementData[2 * i]) < 0)
      @        && (\forall int i; 0 <= i && 2 * i + 1 < elementData.length; elementData[i].compareTo(elementData[2 * i + 1]) < 0)
      @        && (\forall int i; 0 < i && 2 * i + 1 < elementData.length; elementData[2 * i].compareTo(elementData[2 * i + 1]) < 0);
      @*/ 
    ...
    /*@ public normal_behavior
      ...
      @*/
    protected void setElementData(int index, Object element) { //规格要求
